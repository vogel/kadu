...
QFacebookDownloadUnreadThreadsJob::QFacebookDownloadUnreadThreadsJob(QFacebookHttpApi &httpApi, QFacebookSessionToken facebookSessionToken, int unreadCount, QObject *parent) :
		QObject{parent},
		m_httpApi{httpApi},
...
void QFacebookDownloadUnreadThreadsJob::replyFinished(const std::experimental::optional<QFacebookJsonReader> &result) try
{
	deleteLater();
	...
	std::transform(std::begin(nodes), std::end(nodes), std::back_inserter(unreadThreadsResult.unreadThreads), [](const QFacebookJsonReader &v){
		auto threadKey = v.readObject("thread_key");
		return QFacebookDownloadUnreadThreadResult{
			threadKey.readString("other_user_id").toLongLong(),
