...
FacebookChatService::FacebookChatService(Account account, QFacebookSession &facebookSession, QObject *parent) :
		...
		m_facebookSession{facebookSession}
{
	connect(&m_facebookSession, &QFacebookSession::threadsReceived,
			this, &FacebookChatService::threadsReceived);
	connect(&m_facebookSession, &QFacebookSession::unreadThreadsReceived,
			this, &FacebookChatService::unreadThreadsReceived);
	connect(&m_facebookSession, &QFacebookSession::unreadMessagesReceived,
			this, &FacebookChatService::unreadMessagesReceived);
	connect(&m_facebookSession, &QFacebookSession::sendMessageResponseReceived,
			this, &FacebookChatService::sendMessageResponseReceived);
	connect(&m_facebookSession, &QFacebookSession::messageNotificationsReceived,
			this, &FacebookChatService::messageNotificationsReceived);
...
void FacebookChatService::messageNotificationsReceived(const QFacebookPublishOrcaMessageNotifications &orcaMessageNotifications)
{
	...
	auto contact = m_contactManager->byId(account(), QString::number(orcaMessageNotifications.otherUserFbid), ActionCreateAndAdd);
	...
	message.setType(orcaMessageNotifications.otherUserFbid == orcaMessageNotifications.senderFbid
			? MessageType::MessageTypeReceived
			: MessageType::MessageTypeSent);
	message.setMessageSender(orcaMessageNotifications.otherUserFbid == orcaMessageNotifications.senderFbid
			? contact
