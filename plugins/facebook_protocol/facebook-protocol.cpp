...
void FacebookProtocol::facebookLoginFinished(QFacebookLogin::Result &&result)
{
	...
	connect(facebookSession.get(), &QFacebookSession::disconnected,
			this, &FacebookProtocol::facebookDisconnected);
	connect(facebookSession.get(), &QFacebookSession::invalidDataReceived,
			this, &FacebookProtocol::invalidDataReceived);
	connect(facebookSession.get(), &QFacebookSession::presenceReceived,
			this, &FacebookProtocol::presenceReceived);
...
void FacebookProtocol::presenceReceived(const QFacebookPublishPresence &presence)
{
	for (auto const &p : presence.presence)
	{
		...
		newStatus.setType(p.type == QFacebookPresenceType::Online
				? StatusType::Online
				: StatusType::Offline);
