...
void FacebookEditAccountWidget::createGui()
{
    ...
    auto removeAccount =
        make_owned<QPushButton>(qApp->style()->standardIcon(QStyle::SP_DialogCancelButton), tr("Delete account"), this);
    ...
    connect(
        stateNotifier(), &ConfigurationValueStateNotifier::stateChanged, this,
        &FacebookEditAccountWidget::stateChangedSlot);
}
...
void FacebookEditAccountWidget::dataChanged()
{
    ...
    if (account().accountIdentity() == m_identities->currentIdentity() && account().id() == m_accountId->text() &&
        account().rememberPassword() == m_rememberPassword->isChecked() && account().password() == m_password->text() &&
    ...
    if (m_accountId->text().isEmpty() || sameIdExists || StateChangedDataInvalid == widgetsState)
