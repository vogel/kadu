...
OtrRawMessageTransformer::OtrRawMessageTransformer() : EnableFragments{false}
{
}
...
RawMessage OtrRawMessageTransformer::transformReceived(const RawMessage &rawMessage, const Message &message)
{
    ...
    auto ignoreMessage = otrl_message_receiving(
        userState, AppOpsService.data()->appOps(), &opData, account.id().toUtf8().data(),
        account.protocolName().toUtf8().data(), message.messageSender().id().toUtf8().data(),
        rawMessage.rawXmlContent().data(), &newMessage, &tlvs, 0, 0, 0);
...
RawMessage OtrRawMessageTransformer::transformSent(const RawMessage &rawMessage, const Message &message)
{
    ...
    auto err = otrl_message_sending(
        userState, AppOpsService.data()->appOps(), &opData, account.id().toUtf8().data(),
        account.protocolName().toUtf8().data(), receiver.id().toUtf8().data(), OTRL_INSTAG_BEST,
        rawMessage.rawXmlContent().data(), 0, &newMessage,
        EnableFragments ? OTRL_FRAGMENT_SEND_ALL_BUT_LAST : OTRL_FRAGMENT_SEND_SKIP, 0, 0, 0);

    if (!err && newMessage)
    {
        auto transformed = QByteArray{newMessage};
        auto result =
            (rawMessage.rawXmlContent() == transformed.trimmed()) ? rawMessage : RawMessage{transformed, transformed};
