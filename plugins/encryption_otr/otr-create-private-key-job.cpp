...
void OtrCreatePrivateKeyJob::createPrivateKey()
{
    ...
    gcry_error_t err = otrl_privkey_generate_start(
        UserStateService->userState(), MyAccount.id().toUtf8().data(), MyAccount.protocolName().toUtf8().data(),
        &KeyPointer);
    if (err)
    {
        emit finished(MyAccount, false);
...
void OtrCreatePrivateKeyJob::workerFinished(bool ok)
{
    ...
    gcry_error_t err =
        otrl_privkey_generate_finish(UserStateService->userState(), KeyPointer, PrivateStoreFileName.toUtf8().data());
    KeyPointer = 0;
