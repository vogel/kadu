...
void OtrPeerIdentityVerificationFingerprintExchangePage::createGui()
{
    ...
    FingerprintExchangeNotConfirm =
        new QRadioButton(tr("I'm not sure if above Key Fingerprint belongs to %1").arg(MyContact.display(true)));
    FingerprintExchangeConfirm =
        new QRadioButton(tr("I confirm that above Key Fingerprint belongs to %1").arg(MyContact.display(true)));
...
void OtrPeerIdentityVerificationFingerprintExchangePage::setFingerprintService(
