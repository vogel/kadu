...
void OtrPeerIdentityVerificationSelectMethodPage::createGui()
{
    ...
    layout->addWidget(
        createDescirptionLabel(tr("Ask peer a question that only you and he/she know the answer for. If your peer "
                                  "answer it properly, he/she will be marked as verified.")),
        1, 1, 1, 1);
    layout->setRowMinimumHeight(2, 8);
    layout->addWidget(sharedSecread, 3, 0, 1, 2);
    layout->addWidget(
        createDescirptionLabel(tr("Exchange a secret message with peer. This message should be agreed to on another "
                                  "secure channel (face-to-face conversation or a phone).")),
        4, 1, 1, 1);
    layout->setRowMinimumHeight(5, 8);
    layout->addWidget(fingerprintExchange, 6, 0, 1, 2);
    layout->addWidget(
        createDescirptionLabel(tr("Check your peer's fingerprint manually. Fingreprint information shoud be exchanged "
                                  "on another secure channel (face-to-face conversation or a phone).")),
        7, 1, 1, 1);
