...
JabberChatStateService::JabberChatStateService(QXmppClient *client, Account account, QObject *parent)
        : ChatStateService{account, parent}, m_client{client}
{
}
...
QXmppMessage JabberChatStateService::withSentChatState(QXmppMessage message)
{
    ...
    contact.addProperty(
        "jabber:sent-chat-state", static_cast<int>(QXmppMessage::State::Active), CustomProperties::NonStorable);
    return message;
}
...
void JabberChatStateService::sendState(const Contact &contact, ChatState state)
{
    ...
    auto receivedChatState = static_cast<QXmppMessage::State>(
        contact.property("jabber:received-chat-state", QXmppMessage::State::None).toInt());
    ...
    if (!accountData.sendGoneNotification() &&
        (xmppState == QXmppMessage::State::Gone || xmppState == QXmppMessage::State::Inactive))
        xmppState = QXmppMessage::State::Paused;

    auto sentChatState =
        static_cast<QXmppMessage::State>(contact.property("jabber:sent-chat-state", QXmppMessage::State::None).toInt());
    // invalid transition
    if (sentChatState == QXmppMessage::State::None &&
        (xmppState != QXmppMessage::State::Active && xmppState != QXmppMessage::State::Composing &&
