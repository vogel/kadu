...
void JabberRegisterAccount::start()
{
    ...
    */
    ...
    connect(
        m_registerExtension.get(), SIGNAL(registerIqReceived(QXmppRegisterIq)), this,
    ...
    m_pluginInjectedFactory->makeInjected<JabberSslHandler>(
        m_client, connectToServer, [&]() { handleError(tr("SSL certificate rejected")); });
    connectToServer();
}
...
void JabberRegisterAccount::handleRegistrationForm(const QXmppRegisterIq &registerIq)
{
    ...
    {
        auto errorMessage =
            tr("Registration at this server requires XMPP Data Forms support. Kadu currently does not support XMPP "
               "Data Forms.");
