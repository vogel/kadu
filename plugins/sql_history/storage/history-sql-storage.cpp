...
void HistorySqlStorage::init()
{
    if (!QSqlDatabase::isDriverAvailable("QSQLITE"))
    {
        MessageDialog::show(
            ...
            tr("It seems your Qt library does not provide support for selected database. "
               "Please install Qt with %1 plugin.")
                .arg("QSQLITE"));
        return;
    }
    ...
    connect(
        initializer, SIGNAL(progressMessage(QString, QString)), this,
        SLOT(initializerProgressMessage(QString, QString)));
    connect(
...
void HistorySqlStorage::databaseReady(bool ok)
{
    ...
    {
        initializerProgressFinished(
            false, "dialog-error", tr("Opening database failed. Error message:\n%1").arg(Database.lastError().text()));
    ...
    ChatsMapping =
        m_pluginInjectedFactory->makeInjected<SqlChatsMapping>(Database, AccountsMapping, ContactsMapping, this);
...
void HistorySqlStorage::initQueries()
{
    ...
    AppendMessageQuery.prepare(
        "INSERT INTO kadu_messages (chat_id, contact_id, send_time, receive_time, date_id, is_outgoing, content_id) "
        "VALUES "
        "(:chat_id, :contact_id, :send_time, :receive_time, :date_id, :is_outgoing, :content_id)");

    AppendStatusQuery = QSqlQuery(Database);
    AppendStatusQuery.prepare(
...
QString HistorySqlStorage::talkableContactsWhere(const Talkable &talkable)
{
    if (talkable.isValidBuddy())
        ...
        return QString("contact_id = %1")
            .arg(ContactsMapping->idByContact(m_talkableConverter->toContact(talkable), true));
...
void HistorySqlStorage::appendMessage(const Message &message)
{
    ...
    int outgoing = (message.type() == MessageTypeSent) ? 1 : 0;
...
void HistorySqlStorage::clearChatHistory(const Talkable &talkable, const QDate &date)
{
    ...
    QSqlQuery query(Database);
    QString queryString =
        "DELETE FROM kadu_messages WHERE chat_id IN " + chatIdList(m_talkableConverter->toChat(talkable));
    if (!date.isNull())
    ...
    QString removeChatsQueryString =
        "DELETE FROM kadu_chats WHERE 0 = (SELECT COUNT(*) FROM kadu_messages WHERE chat_id = kadu_chats.id)";
...
QVector<HistoryQueryResult> HistorySqlStorage::syncChatDates(const HistoryQuery &historyQuery)
{
    ...
    queryString +=
        " (SELECT km.rowid, date, date_id, chat_id, content FROM kadu_messages km "
...
QVector<HistoryQueryResult> HistorySqlStorage::syncStatusDates(const HistoryQuery &historyQuery)
{
    ...
    queryString +=
        " (SELECT set_time, contact_id, description FROM kadu_statuses WHERE " + talkableContactsWhere(talkable);
...
SortedMessages HistorySqlStorage::syncMessages(const HistoryQuery &historyQuery)
{
    ...
    QSqlQuery query(Database);
    QString queryString =
        "SELECT chat_id, contact_id, kmc.content, send_time, receive_time, is_outgoing FROM kadu_messages "
...
SortedMessages HistorySqlStorage::syncStatuses(const HistoryQuery &historyQuery)
{
    ...
    QSqlQuery query(Database);
    QString queryString =
        "SELECT contact_id, status, description, set_time FROM kadu_statuses WHERE " + talkableContactsWhere(talkable);
