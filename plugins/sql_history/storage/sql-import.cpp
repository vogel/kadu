...
void SqlImport::initV4Tables(QSqlDatabase &database)
{
    ...
    query.prepare("ALTER TABLE kadu_contacts ADD COLUMN contact VARCHAR(1024)");
    query.exec();
    ...
    query.prepare("ALTER TABLE kadu_chats ADD COLUMN chat TEXT");
    query.exec();
...
void SqlImport::importChatsToV4(QSqlDatabase &database)
{
    ...
    auto chatsMapping =
        m_pluginInjectedFactory->makeUnique<SqlChatsMapping>(database, accountsMapping.get(), contactsMapping.get());
...
void SqlImport::dropBeforeV4Fields(QSqlDatabase &database)
{
    ...
    queries
        ...
        << "CREATE TABLE kadu_statuses ("
           ...
           "description TEXT"
           ");"
        << "INSERT INTO kadu_statuses (contact_id, status, set_time, description) SELECT contact_id, status, set_time, "
           "description FROM kadu_statuses_old;"
...
void SqlImport::dropBeforeV4Indexes(QSqlDatabase &database)
{
    ...
    QStringList queries;
    queries << "DROP INDEX IF EXISTS kadu_chat_uuid;"
...
void SqlImport::importVersion1Schema(QSqlDatabase &database)
{
    ...
    queries
        ...
        << "CREATE TABLE kadu_message_contents (id INTEGER PRIMARY KEY AUTOINCREMENT, content TEXT, attributes "
           "VARCHAR(25));"
        ...
        << "INSERT INTO kadu_dates (date) SELECT DISTINCT REPLACE(substr(receive_time,0,11), '-', '') FROM "
           "kadu_messages_old;"
        ...
        << "CREATE TABLE kadu_messages ("
           ...
           "FOREIGN KEY (date_id) REFERENCES kadu_dates(id), FOREIGN KEY (content_id) REFERENCES "
           "kadu_message_contents(id));"

        << "INSERT INTO kadu_messages ("
           "chat_id, contact_id, date_id, send_time, receive_time, content_id, is_outgoing) "
           "SELECT (SELECT id FROM kadu_chats WHERE uuid=old.chat LIMIT 1), (SELECT id FROM kadu_contacts WHERE "
           "uuid=old.sender LIMIT 1), "
           "(SELECT id FROM kadu_dates WHERE date = REPLACE(substr(old.receive_time,0,11), '-', '')), send_time, "
           "receive_time, "
           "(SELECT id FROM kadu_message_contents WHERE content=old.content LIMIT 1), substr(attributes, 10, 1) FROM "
           "kadu_messages_old old;"
...
void SqlImport::removeDuplicatesFromVersion2Schema(
