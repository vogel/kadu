...
{
	QFileInfo scheme0FileInfo(PathsProvider::instance()->profilePath() + QLatin1String(HISTORY_FILE_0));
	QFileInfo scheme1FileInfo(PathsProvider::instance()->profilePath() + QLatin1String(HISTORY_FILE_1));
	return scheme0FileInfo.exists() || scheme1FileInfo.exists();
}

bool SqlInitializer::currentHistoryFileExists()
{
	QFileInfo schemeCurrentFileInfo(PathsProvider::instance()->profilePath() + QLatin1String(HISTORY_FILE_CURRENT));
...
void SqlInitializer::initDatabase()
{
	...
	{
		emit progressMessage("dialog-information", tr("Copying history file to new location: %1 ...").arg(PathsProvider::instance()->profilePath() + QLatin1String(HISTORY_FILE_CURRENT)));
		if (!copyHistoryFile())
		{
	...
	QString historyFilePath = PathsProvider::instance()->profilePath() + QLatin1String(HISTORY_FILE_CURRENT);
