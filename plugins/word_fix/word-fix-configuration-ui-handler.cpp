...
WordFixConfigurationUiHandler::WordFixConfigurationUiHandler(QObject *parent) : QObject{parent}
{
}
...
void WordFixConfigurationUiHandler::mainConfigurationWindowCreated(MainConfigurationWindow *mainConfigurationWindow)
{
    ...
    for (QMap<QString, QString>::const_iterator i = m_wordFix->wordsList().constBegin();
         i != m_wordFix->wordsList().constEnd(); ++i)
    {
...
void WordFixConfigurationUiHandler::mainConfigurationWindowApplied()
{
    ...
    if (item)
    {
        QString wordStr = item->text(0), valueStr = item->text(1);
...
void WordFixConfigurationUiHandler::changeSelected()
{
    ...
    QString wordStr = m_wordEdit->text(), valueStr = m_valueEdit->text();
