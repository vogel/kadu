...
SingleWindow::SingleWindow(QWidget *parent) : QWidget{parent}
{
}
...
void SingleWindow::init()
{
    ...
    new WindowGeometryManager(
        new ConfigFileVariantWrapper(m_configuration, "SingleWindow", "WindowGeometry"), QRect(0, 0, 800, 440), this);
...
ChatWidget *SingleWindow::addChat(Chat chat, OpenChatActivation activation)
{
    ...
    connect(
        chatWidget->edit(), SIGNAL(keyPressed(QKeyEvent *, CustomInput *, bool &)), this,
        SLOT(onChatKeyPressed(QKeyEvent *, CustomInput *, bool &)));
