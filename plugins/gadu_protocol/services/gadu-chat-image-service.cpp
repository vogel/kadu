...
void GaduChatImageService::setGaduChatService(GaduChatService *gaduChatService)
{
    ...
    if (CurrentChatService)
        connect(
            CurrentChatService.data(), SIGNAL(chatImageKeyReceived(QString, ChatImage)), this,
            SLOT(chatImageKeyReceivedSlot(QString, ChatImage)));
}
...
void GaduChatImageService::handleEventImageRequest(struct gg_event *e)
{
    ...
    gg_image_reply(
        writableSessionToken.rawSession(), e->event.image_request.sender, image.key().toUtf8().constData(),
...
{
    quint32 crc32 =
        imageData.isEmpty() ? 0 : gg_crc32(0, (const unsigned char *)imageData.constData(), imageData.length());
...
Error GaduChatImageService::checkImageSize(qint64 size) const
{
    ...
    QString message = tr("This image has %1 KiB and exceeds recommended maximum size of %2 KiB. Some clients may have "
                         "trouble with too large images.") +
