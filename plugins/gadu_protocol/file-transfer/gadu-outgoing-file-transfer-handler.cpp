...
GaduOutgoingFileTransferHandler::GaduOutgoingFileTransferHandler(GaduProtocol *protocol, FileTransfer fileTransfer)
        : OutgoingFileTransferHandler{fileTransfer}, m_protocol{protocol}
{
}
...
void GaduOutgoingFileTransferHandler::send(QIODevice *source)
{
    ...
    auto sendTicketRequest =
        driveService->requestSendTicket(contact.id(), transfer().remoteFileName(), transfer().fileSize());
    connect(
        sendTicketRequest, SIGNAL(sendTickedReceived(GaduDriveSendTicket)), this,
        SLOT(initialStatusUpdateReceived(GaduDriveSendTicket)));
...
void GaduOutgoingFileTransferHandler::startOutgoingTransferIfNotStarted()
{
    ...
    connect(
        m_putTransfer, &GaduDrivePutTransfer::finished, this,
        &GaduOutgoingFileTransferHandler::requestSendStatusUpdate);
}
...
void GaduOutgoingFileTransferHandler::requestSendStatusUpdate()
{
    ...
    connect(
        updateSendStatusRequest, SIGNAL(statusUpdateReceived(GaduDriveSendTicket)), this,
        SLOT(statusUpdateReceived(GaduDriveSendTicket)));
}
