...
QMessagingMenuSource::QMessagingMenuSource(
    const QString &id, const QString &label, const QString &icon, QMessagingMenuApp *menuApp)
        : QObject{menuApp}, m_menuApp{menuApp}, m_id{id}
{
    Q_ASSERT(m_menuApp);
    Q_UNUSED(icon);
        ...
        : QObject{menuApp}, m_menuApp{menuApp}, m_id{id}
{
    ...
    auto gicon = !icon.isEmpty() ? g_icon_new_for_string(icon.toUtf8().data(), nullptr) : nullptr;
    messaging_menu_app_append_source_with_count(
        m_menuApp->app(), m_id.toUtf8().data(), gicon, label.toUtf8().data(), count);
    if (gicon)
