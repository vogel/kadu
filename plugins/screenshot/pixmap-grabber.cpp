...
Window PixmapGrabber::windowUnderCursor(bool includeDecorations)
{
    ...
    XGrabServer(QX11Info::display());
    XQueryPointer(QX11Info::display(), QX11Info::appRootWindow(), &root, &child, &rootX, &rootY, &winX, &winY, &mask);
    if (child == None)
...
void PixmapGrabber::getWindowsRecursive(std::vector<QRect> &windows, Window w, int rx, int ry, int depth)
{
    ...
    if (atts.map_state == IsViewable && atts.width >= MINIMUM_SIZE && atts.height >= MINIMUM_SIZE)
    {
        int x = 0, y = 0;
        if (depth)
        {
...
Window PixmapGrabber::findRealWindow(Window w, int depth)
{
    ...
    if (XGetWindowProperty(
            QX11Info::display(), w, wm_state, 0, 0, False, AnyPropertyType, &type, &format, &nitems, &after, &prop) ==
        Success)
    {
        if (prop != NULL)
            XFree(prop);
