...
void TabsManager::init()
{
    ...
    connect(
        m_chatWidgetRepository, SIGNAL(chatWidgetRemoved(ChatWidget *)), this, SLOT(onDestroyingChat(ChatWidget *)));
    ...
    connect(
        TabDialog, SIGNAL(contextMenu(QWidget *, const QPoint &)), this,
        SLOT(onContextMenu(QWidget *, const QPoint &)));

    connect(Title, SIGNAL(titleChanged()), this, SLOT(onTitleChanged()));

    new WindowGeometryManager(
        new ConfigFileVariantWrapper(m_configuration, "Chat", "TabWindowsGeometry"), QRect(30, 30, 550, 400),
        TabDialog);
    ...
    m_menuInventory->menu("buddy-list")->addAction(m_openInNewTabAction, KaduMenu::SectionChat, 20)->update();
...
void TabsManager::done()
{
    ...
    m_menuInventory->menu("buddy-list")->removeAction(m_openInNewTabAction)->update();
...
void TabsManager::insertTab(ChatWidget *chatWidget)
{
    ...
    connect(
        chatWidget->edit(), SIGNAL(keyPressed(QKeyEvent *, CustomInput *, bool &)), TabDialog,
