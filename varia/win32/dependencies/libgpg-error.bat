...
if exist libgpg-error-%GPGERRORVER% %RMDIR% libgpg-error-%GPGERRORVER%
...
if exist libgpg-error-%GPGERRORVER%.tar %RM% libgpg-error-%GPGERRORVER%.tar
%SEVENZ% x libgpg-error-%GPGERRORVER%.tar.bz2
if errorlevel 1 exit /b 1
%SEVENZ% x libgpg-error-%GPGERRORVER%.tar
if errorlevel 1 exit /b 1
%RM% libgpg-error-%GPGERRORVER%.tar
...
%CP% "%INSTALLPREFIX%"\libgpg-error-install\bin\libgpg-error.dll "%INSTALLBASE%"
if errorlevel 1 exit /b 1
