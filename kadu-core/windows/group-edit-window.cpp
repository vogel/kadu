...
GroupEditWindow::GroupEditWindow(Group group, QWidget *parent)
        :   // using C++ initializers breaks Qt's lupdate
          QDialog(parent),
          m_group(group), m_add(group.isNull())
{
    setAttribute(Qt::WA_DeleteOnClose, true);
    setMinimumWidth(400);
    setWindowRole("kadu-group-edit");
    setWindowTitle(m_add ? tr("Add group") : tr("Edit group %1").arg(group.name()));
}
...
void GroupEditWindow::createGui()
{
    ...
    auto saveTitle = m_add ? tr("Add group") : tr("Save group");
...
void GroupEditWindow::selectIcon()
{
    ...
    auto file = QFileDialog::getOpenFileName(
        this, tr("Choose an icon"), recentPath, tr("Images (*.png *.xpm *.jpg);;All Files (*)"));
...
void GroupEditWindow::dataChanged()
{
    ...
    if (m_nameLineEdit->text() != m_group.name() || m_nameCheckBox->isChecked() != m_group.showName() ||
        m_iconCheckBox->isChecked() != m_group.showIcon() || m_selectedIcon != m_group.icon() ||
