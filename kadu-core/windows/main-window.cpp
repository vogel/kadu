...
MainWindow::MainWindow(ActionContext *context, const QString &windowName, QWidget *parent)
        : QMainWindow(parent), DesktopAwareObject(this), WindowName(windowName), TransparencyEnabled(false),
          BlurEnabled(false), Context(context)
{
}
...
{
    connect(
        m_configurationManager->toolbarConfigurationManager(), SIGNAL(configurationUpdated()), this,
        SLOT(refreshToolBars()));
    connect(
        m_actions, SIGNAL(actionLoaded(ActionDescription *)), this, SLOT(actionLoadedOrUnloaded(ActionDescription *)));
    connect(
        m_actions, SIGNAL(actionUnloaded(ActionDescription *)), this,
        SLOT(actionLoadedOrUnloaded(ActionDescription *)));
}
...
bool MainWindow::loadOldToolBarsFromConfig(const QString &configName, Qt::ToolBarArea area)
{
    ...
    QDomElement dockareaConfig =
        m_configuration->api()->findElementByProperty(toolbarsConfig, "DockArea", "name", configName);
...
QDomElement
MainWindow::getDockAreaConfigElement(Configuration *configuration, QDomElement toolbarsConfig, const QString &name)
{
...
void MainWindow::addToolButton(
    Configuration *configuration, QDomElement toolbarConfig, const QString &actionName, Qt::ToolButtonStyle style)
{
    QDomElement buttonConfig =
        configuration->api()->findElementByProperty(toolbarConfig, "ToolButton", "action_name", actionName);
...
{
    QDomElement dockAreaConfig = configuration->api()->findElementByProperty(
        getToolbarsConfigElement(configuration), "DockArea", "name", areaName);
...
QDomElement MainWindow::findExistingToolbar(Configuration *configuration, const QString &prefix)
{
    ...
    QDomElement dockAreaConfig =
        getDockAreaConfigElement(configuration, getToolbarsConfigElement(configuration), realPrefix + "topDockArea");
...
Contact MainWindow::contact()
{
    ...
    return 1 == contactSet.count() ? *contactSet.constBegin() : Contact::null;
}

Buddy MainWindow::buddy()
{
    BuddySet buddySet = actionContext()->buddies();
    return 1 == buddySet.count() ? *buddySet.constBegin() : Buddy::null;
}
