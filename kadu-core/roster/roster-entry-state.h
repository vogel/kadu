 ...
 *
 * This enum describes state of roster entry that can be either synchronized, desynchronized or being synchronizing with
 * remote entry.
 */
enum class RosterEntryState
{
    ...
    Unknown,
    /**
     * Entries with this state have the same value on local and remote roster. When remote roster sends an update then
     * local value must be updated as well.
     */
    Synchronized,
    /**
    ...
    /**
     * Entries with this state are not synchronized at all with remote server. However, when one of remote or local
     * rosters removes them
