 ...
 * @link RosterTask @endlink for each contact id can be on the list at a time. This class is responsible of choosing
 * which task
 * should be left on the list and which one should be removed in case when second one is added for given id.
 *
 * When receiving updates from the remote roster, list of tasks is checked for id of contact from remote roster.
 * Requests for
 * contact removal or update from remote roster will be ignored if an update or addition task is on the task list. This
 * service
...
public:
     ...
     *
     * For each contact that has a RosterEntry and is in desynchronized state a new RosterTask with Update mode is
     * returned.
     */
     ...
     * If existing task is deletion, then it is always replaced. Addition task can be only replaced by deletion task.
     * Update task
...
template <typename T>
inline RosterServiceTasks *rosterServiceTasks(T x)
{
