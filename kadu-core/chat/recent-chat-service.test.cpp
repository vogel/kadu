...
public:
    ...
    virtual Chat byDisplay(const QString &) override
    {
        return {};
    }
    virtual QVector<Chat> chats(const Account &) override
    {
        return {};
    }
    virtual Chat loadStubFromStorage(const std::shared_ptr<StoragePoint> &) override
    {
        return {};
    }
...
public:
    ...
    virtual bool sendMessage(const Chat &, NormalizedHtmlString, bool) override
    {
        return {};
    }
    virtual bool sendRawMessage(const Chat &, const QByteArray &) override
    {
        return {};
    }
...
public:
    ...
    virtual std::unique_ptr<StoragePoint> createStoragePoint(const QString &, StoragePoint *)
    {
        return {};
    }
};
...
void RecentChatServiceTest::shouldIncludeLoadedChatWithRecentMessage()
{
    ...
    chat1.addProperty(
        RecentChatService::LAST_MESSAGE_DATE_TIME_PROPERTY, QDateTime::currentDateTimeUtc(),
    ...
    QCOMPARE(recentChatRepository->size(), size_t{1});
    QVERIFY(
        std::find(recentChatRepository->begin(), recentChatRepository->end(), chat1) != recentChatRepository->end());
    QVERIFY(
        std::find(recentChatRepository->begin(), recentChatRepository->end(), chat2) == recentChatRepository->end());
}
...
void RecentChatServiceTest::shouldNotIncludeLoadedChatWithTooOldRecentMessage()
{
    Chat chat{new ChatShared{}};
    chat.addProperty(
        RecentChatService::LAST_MESSAGE_DATE_TIME_PROPERTY,
        QDateTime::currentDateTimeUtc().addSecs(-1 * RecentChatService::KEEP_RECENT_FOR_SECONDS - 1),
        CustomProperties::Storable);
