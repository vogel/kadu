...
public:
     ...
     *
     * This functions returns roaming persistent data storage path (%AppData%) on Windows and QDir::homePath() on other
     * platforms.
     */
     ...
     * @todo Check whether this is really needed. Maybe all paths should start with "file://" and it could work
     * everywhere?
     *
     ...
     * arguments.
     */
    const QString &desktopFilePath() const
    {
     ...
     * current user profile is located in the same location as dataPath() + QStringLiteral("config")
