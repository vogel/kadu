...
public:
     ...
     */
    sorted_unique_vector(Storage storage) : m_content{std::move(storage)}
    {
    ...
    void merge(const This &sortedVector)
    {
        ...
        std::merge(
            std::begin(m_content), std::end(m_content), std::begin(sortedVector.m_content),
            std::end(sortedVector.m_content), std::back_inserter(result), LessThanComparator);
        ensureUnique(result);
