...
std::set<typename GraphType::PayloadType> graph_find_cycles(const GraphType &graph)
{
    ...
    std::transform(
        cycleNodes.begin(), cycleNodes.end(), std::inserter(result, result.begin()),
        [](typename GraphType::NodePointer node) { return node->payload(); });
...
std::vector<typename GraphType::NodePointer>
graph_sort_successors(const GraphType &graph, const PayloadType &payload) noexcept(false)
{
