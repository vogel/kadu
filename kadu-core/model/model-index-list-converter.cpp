...
ModelIndexListConverter::ModelIndexListConverter(const QModelIndexList &modelIndexList, QObject *parent)
        : QObject{parent}, ModelIndexList(modelIndexList)
{
}
...
Chat ModelIndexListConverter::chatFromIndex(const QModelIndex &index) const
{
    ...
    case ContactRole:
        return m_unreadMessageRepository->unreadMessageForContact(index.data(ContactRole).value<Contact>())
            .messageChat();
    }
...
Chat ModelIndexListConverter::chatFromBuddies() const
{
    ...
    if (1 == buddies.size())
        return ChatTypeContact::findChat(
            m_chatManager, m_chatStorage, m_buddyPreferredManager->preferredContact2(*buddies.begin()),
            ActionCreateAndAdd);
    else
        return ChatTypeContactSet::findChat(
            m_chatManager, m_chatStorage, m_buddyPreferredManager->preferredContacts(buddies), ActionCreateAndAdd);
}
