 ...
 *
 * StatusContainerManager is also a status container that delegates all of its getters to default status container
 * (first
...
class KADUAPI StatusContainerManager : public StatusContainer,
                                       private AccountsAwareObject,
                                       private IdentitiesAwareObject
{
    Q_OBJECT
     ...
     *
     * This method registers new status container. Signal statusUpdated of this container is connected to the same
     * signal of
     ...
     *
     * This method unregisters status container. Signal statusUpdated of this container is disconnected from the same
     * signal of
     * StatusContainerManager. Signals statusContainerAboutToBeUnregistered and statusContainerUnregistered are emited.
     * New default
     * status container may be set, if statusContainer was the default one. Empty default status container may be set,
     * if no more
...
private slots:
     ...
     * an account
     * changed its identity or so on. When StatusContainerManager is in one container per identity mode this methods
     * updates list
...
public:
     ...
     */
    const QList<StatusContainer *> &statusContainers() const
    {
     ...
     *
     * Returns status of default status container. If no default status container is available, returns "Offline"
     * status.
     */
     ...
     *
     * Returns "is status setting in progress" property of default status container. If no default status container is
     ...
     *
     * Returns generic (without protocol badge) icon of status of default status container. If no default status
     ...
     *
     * Returns list of default status container supported status types. If no default status container is available,
     ...
     *
     * Returns maximum description length of default status container. If no default status container is available, -1
     * is returned.
     */
     ...
     *
     * Returns stored status of default status container.If no default status container is available, "Offline" status
     * is returned.
     */
     ...
     *
     * Depending on settings it can return account's status container, its identity's status container or global status
     * container.
     */
