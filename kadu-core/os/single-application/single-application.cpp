...
SingleApplication::SingleApplication(
    ...
    QObject *parent)
        : QObject{parent}, m_localServer{nullptr}, m_onReceiveMessage{onReceiveMessage}
{
...
{
    return QString(QDir{QDir::tempPath()}.absolutePath() + QLatin1Char{'/'} + socketName + QStringLiteral("-lockfile"));
}
...
void SingleApplication::startServer()
{
    ...
    {
        qWarning(
            "QtSingleCoreApplication: listen on local socket failed, %s", qPrintable(m_localServer->errorString()));
        return;
    }
