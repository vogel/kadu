...
void UrlOpener::openUrl(const QString &url)
{
	...
	if (!openUrl(url, url, browser))
		MessageDialog::msg(qApp->translate("@default", QT_TR_NOOP("Could not spawn Web browser process. Check if the Web browser is functional")), false, "dialog-error_big.png");
}
...
void UrlOpener::openEmail(const QString &email)
{
	...
	if (!openUrl(urlForDesktopServices, urlForApplication, client))
		MessageDialog::msg(qApp->translate("@default", QT_TR_NOOP("Could not spawn Mail client process. Check if the Mail client is functional")), false, "dialog-error_big.png");
}
