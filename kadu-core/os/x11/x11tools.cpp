...
bool X11_getCardinalProperty(Display *display, Window window, const char *propertyName, uint32_t *value, long offset)
{
    ...
    int result = XGetWindowProperty(
        display, window, property, offset, 1L, False, XA_CARDINAL, &realtype, &realformat, &nitems, &left,
        (unsigned char **)&data);
    if (result == Success)
    {
...
bool X11_getFirstPropertyAtom(Display *display, Window window, const char *propertyName, Atom *value)
{
    ...
    unsigned long nitems, left;
    int result = XGetWindowProperty(
        display, window, property, 0L, 1L, False, XA_ATOM, &realtype, &realformat, &nitems, &left,
        (unsigned char **)&data);
    if (result == Success)
    {
        if (realtype == XA_ATOM)
...
bool X11_isPropertyAtomSet(Display *display, Window window, const char *propertyName, const char *atomName)
{
    ...
    unsigned long nitems, left;
    int result = XGetWindowProperty(
        display, window, property, 0L, 8192L, False, XA_ATOM, &realtype, &realformat, &nitems, &left,
        (unsigned char **)&atoms);
    if (result != Success)
        return false;
    if (realtype != XA_ATOM)
...
bool X11_isFreeDesktopCompatible(Display *display)
{
    ...
    if (resolution.first == 0)
        return false;
    if ((desktopsize.first % resolution.first != 0) ||
        (desktopsize.second % resolution.second != 0))   // virtual resolution
        return true;
...
uint32_t X11_getCurrentDesktop(Display *display, bool forceFreeDesktop)
{
    if ((!forceFreeDesktop) && (!X11_isFreeDesktopCompatible(display)))
    {
        ...
        if (resolution.second == 0)
            return 0;
        uint32_t desktop = static_cast<uint32_t>(
            (static_cast<int>(dy) / resolution.second) * (desktopsize.first / resolution.first) +
            (static_cast<int>(dx) / resolution.first));
        return desktop;
    }
    else
    {
...
void X11_setCurrentDesktop(Display *display, uint32_t desktop, bool forceFreeDesktop)
{
        ...
        (desktop != X11_getCurrentDesktop(display, forceFreeDesktop)))
    {
        ...
        if ((!forceFreeDesktop) && (!X11_isFreeDesktopCompatible(display)))
        {
            std::pair<int, int> desktopsize = X11_getDesktopSize(display);
            std::pair<int, int> resolution = X11_getResolution(display);
            uint32_t dx = (desktop % (static_cast<uint32_t>(desktopsize.first / resolution.first))) *
                          static_cast<uint32_t>(resolution.first);
            uint32_t dy = (desktop / (static_cast<uint32_t>(desktopsize.first / resolution.first))) *
            ...
            XSendEvent(
                display, DefaultRootWindow(display), False, SubstructureRedirectMask | SubstructureNotifyMask, &xev);
            XFlush(display);
        }
        else
        {
...
uint32_t X11_getDesktopOfWindow(Display *display, Window window, bool forceFreeDesktop, bool windowareadecides)
{
    if ((!forceFreeDesktop) && (!X11_isFreeDesktopCompatible(display)))
    {
        ...
        uint32_t desktopofwindow =
            currentdesktop + static_cast<uint32_t>(
                                 (pos.second / resolution.second) * (desktopsize.first / resolution.first) +
                                 (pos.first / resolution.first));
        if (pos.first < 0)
            desktopofwindow -= 1;
        if (pos.second < 0)
...
void X11_moveWindowToDesktop(
    Display *display, Window window, uint32_t desktop, bool forceFreeDesktop, bool position, int x, int y)
{
    if ((!forceFreeDesktop) && (!X11_isFreeDesktopCompatible(display)))
    {
        std::pair<int, int> pos = X11_getWindowPos(display, window);
        ...
        uint32_t ddx = (desktop % static_cast<uint32_t>(desktopsize.first / resolution.first)) -
                       (desktopofwindow % static_cast<uint32_t>(desktopsize.first / resolution.first));
        uint32_t ddy = (desktop / static_cast<uint32_t>(desktopsize.first / resolution.first)) -
                       (desktopofwindow / static_cast<uint32_t>(desktopsize.first / resolution.first));
        int newx, newy;
        if (position)
        {
    ...
    if (children != NULL)
    {
        ...
        for (; k < nchildren; k++)
        {
                ...
                ((y1 <= Y1 && Y1 <= y2) || (y1 <= Y2 && Y2 <= y2) || (y1 >= Y1 && y2 <= Y2)))
            {
                ...
                windows.clear();
                if ((notypes && (!hastype)) ||
                    (hastype && (type != type_dock) && (type != type_toolbar) && (type != type_menu) &&
...
bool X11_isWindowMinimized(Display *display, Window window)
{
    ...
    if (property == None)
        return false;
    if (!XGetWindowProperty(display, window, property, 0, 2, False, property, &r_type, &r_format, &count, &after, &p))
    {
        if (!p)
            return true;
        auto ret = (long)*p == 3;
...
Window X11_getActiveWindow(Display *display)
{
    ...
    if (net_active_window != None)
    {
        ...
        unsigned long nitems, left;
        int result = XGetWindowProperty(
            display, XDefaultRootWindow(display), net_active_window, 0L, sizeof(Window), False, XA_WINDOW, &realtype,
            &realformat, &nitems, &left, (unsigned char **)&windowptr);
        if (result == Success)
        {
...
Window X11_getTopMostWindow(Display *display)
{
    ...
    if (XGetWindowProperty(
            display, DefaultRootWindow(display), listatom, 0L, (~0L), False, XA_WINDOW, &type_return, &format_return,
            &nitems_return, &bytesafter_return, (unsigned char **)&windowarray) == Success)
    {
        Window window = None;
...
Window X11_getWindowUnderCursor(Display *display, int *rootx, int *rooty, int *windowx, int *windowy)
{
    ...
    unsigned int mask;
    XQueryPointer(
        display, root, &root, &window, (rootx ? rootx : &_rootx), (rooty ? rooty : &_rooty),
        (windowx ? windowx : &_windowx), (windowy ? windowy : &_windowy), &mask);
    return window;
}
...
Window X11_getInnerMostWindowUnderCursor(Display *display, int *rootx, int *rooty, int *windowx, int *windowy)
{
    ...
    do
    {
        window = child;
        XQueryPointer(
            display, window, &root, &child, (rootx ? rootx : &_rootx), (rooty ? rooty : &_rooty),
            (windowx ? windowx : &_windowx), (windowy ? windowy : &_windowy), &mask);
...
bool X11_checkFullScreen(Display *display)
{
    ...
    if (wa != None)
        if (X11_isPropertyAtomSet(display, wa, "_NET_WM_STATE", "_NET_WM_STATE_FULLSCREEN"))
            return true;
    _x11toolsdebug("[B]") Window wt = X11_getTopMostWindow(display);
    if (wt != None)
    ...
    if (X11_isPointerGrabbed(display))
    {
        ...
        if (wt != None)
            ...
            {
                _x11toolsdebug("[D2]");
                if (X11_isPropertyAtomSet(display, wt, "_NET_WM_STATE", "_NET_WM_STATE_MAXIMIZED_HORZ") &&
        ...
        if (wl != None)
            if (X11_getWindowSize(display, wl) == resolution)
            {
                ...
                if (X11_isPropertyAtomSet(display, wl, "_NET_WM_STATE", "_NET_WM_STATE_MAXIMIZED_HORZ") &&
        ...
        if (wl != None)
        {
            ...
            if (X11_getFirstPropertyAtom(display, wl, "_NET_WM_WINDOW_TYPE", &wl_type) && (wl_type != None))
            {
                ...
                Atom type_popupmenu = XInternAtom(display, "_NET_WM_WINDOW_TYPE_POPUP_MENU", False);
                Atom type_combo = XInternAtom(display, "_NET_WM_WINDOW_TYPE_COMBO", False);
                if ((wl_type == type_toolbar) || (wl_type == type_menu) || (wl_type == type_dropdownmenu) ||
                    (wl_type == type_popupmenu) || (wl_type == type_combo))
                {
