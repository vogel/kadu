...
void CategorizedListViewPainter::drawCategory(
    const QModelIndex &index, int /*sortRole*/, const QStyleOption &option, QPainter *painter) const
{
    painter->setRenderHint(QPainter::Antialiasing);

    const QString category =
        index.model()->data(index, CategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
...
CategorizedListViewPainter::CategorizedListViewPainter(CategorizedListView *view) : View(view)
{
    setLeftMargin(2);
    setRightMargin(2);
}
