...
{
    return m_blinkTimer && m_blink && m_blinkTitleWhenUnreadMessages ? QString{" "}   // do not emit empty title
                                                                     : m_title;
}

QString ChatWidgetTitle::shortTitle() const
{
    return m_title.length() > 15 ? m_title.left(15) + "..." : m_title;
}
...
{
    auto showMessageIcon =
        m_blinkTimer && m_blinkIconWhenUnreadMessages ? m_blink : chatWidget()->chat().unreadMessagesCount() > 0;
    return showMessageIcon ? m_iconsManager->iconByPath(KaduIcon("protocols/common/message")) : m_icon;
}
...
{
    if ((m_blinkTitleWhenUnreadMessages || m_blinkIconWhenUnreadMessages) &&
...
QString ChatWidgetTitle::withUnreadMessagesCount(QString title) const
{
    ...
    return count > 0 ? QString{"[%1] %2"}.arg(count).arg(title) : title;
}
