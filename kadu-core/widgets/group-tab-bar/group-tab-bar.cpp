...
void GroupTabBar::dropEvent(QDropEvent *event)
{
    ...
    if (tabIndex == -1)
    {
        ...
        do
        {
            newGroupName = QInputDialog::getText(
                this, tr("New Group"), tr("Please enter the name for the new group:"), QLineEdit::Normal, QString(),
                &ok);
    ...
    if (clickedGroup)
    {
        ...
        if (m_groupFilter.filterType() == GroupFilterRegular)
            menu.addAction(tr("Move to group %1").arg(clickedGroup.name()), this, SLOT(moveToGroup()))
                ->setData(clickedGroup);
...
void GroupTabBar::deleteGroup()
{
    ...
    MessageDialog *dialog = MessageDialog::create(
        m_iconsManager->iconByPath(KaduIcon("dialog-warning")), tr("Delete group"),
