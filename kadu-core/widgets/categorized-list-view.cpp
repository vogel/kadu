...
struct CategorizedListViewPrivate::Item
{
    Item() : topLeft(QPoint()), size(QSize())
    {
    }
...
struct CategorizedListViewPrivate::Block
{
    Block()
            : topLeft(QPoint()), height(-1), firstIndex(QModelIndex()), quarantineStart(QModelIndex()),
              items(QList<Item>()), outOfQuarantine(false), alternate(false), collapsed(false)
    {
    }
...
CategorizedListViewPrivate::CategorizedListViewPrivate(CategorizedListView *q)
        : q(q), proxyModel(0), categoryDrawer(0), categorySpacing(5), alternatingBlockColors(false),
          collapsibleBlocks(false), hoveredBlock(new Block()), hoveredIndex(QModelIndex()), pressedPosition(QPoint()),
          rubberBandRect(QRect())
{
}

CategorizedListViewPrivate::~CategorizedListViewPrivate()
{
    delete hoveredBlock;
}

bool CategorizedListViewPrivate::isCategorized() const
{
...
QPair<QModelIndex, QModelIndex> CategorizedListViewPrivate::intersectingIndexesWithRect(const QRect &_rect) const
{
...
QPoint CategorizedListViewPrivate::blockPosition(const QString &category)
{
    ...
    for (QHash<QString, CategorizedListViewPrivate::Block>::Iterator it = blocks.begin(); it != blocks.end(); ++it)
    {
        Block &block = *it;
...
int CategorizedListViewPrivate::blockHeight(const QString &category)
{
    Block &block = blocks[category];
    ...
    const QModelIndex lastIndex =
        proxyModel->index(firstIndex.row() + block.items.count() - 1, q->modelColumn(), q->rootIndex());
...
int CategorizedListViewPrivate::viewportWidth() const
{
    return q->viewport()->width() - categorySpacing * 2 - categoryDrawer->leftMargin() - categoryDrawer->rightMargin();
}

void CategorizedListViewPrivate::regenerateAllElements()
{
...
void CategorizedListViewPrivate::rowsInserted(const QModelIndex &parent, int start, int end)
{
    if (!isCategorized())
    {
        return;
    ...
    for (int i = start; i <= end; ++i)
    {
        ...
        block.items.insert(index.row() - firstIndexRow, CategorizedListViewPrivate::Item());
    ...
    // BEGIN: update the items that are in quarantine in affected categories
    {
        ...
        CategorizedListViewPrivate::Block &block = blocks[category];
        block.quarantineStart = block.firstIndex;
    }
    ...
    // BEGIN: mark as in quarantine those categories that are under the affected ones
    {
        ...
        for (QHash<QString, CategorizedListViewPrivate::Block>::Iterator it = blocks.begin(); it != blocks.end(); ++it)
        {
            CategorizedListViewPrivate::Block &block = *it;
...
QRect CategorizedListViewPrivate::mapToViewport(const QRect &rect) const
{
...
QRect CategorizedListViewPrivate::mapFromViewport(const QRect &rect) const
{
    const int dx = q->horizontalOffset();
    const int dy = q->verticalOffset();
    return rect.adjusted(dx, dy, dx, dy);
...
int CategorizedListViewPrivate::highestElementInLastRow(const Block &block) const
{
    // Find the highest element in the last row
    const QModelIndex lastIndex =
        proxyModel->index(block.firstIndex.row() + block.items.count() - 1, q->modelColumn(), q->rootIndex());
...
bool CategorizedListViewPrivate::hasGrid() const
{
...
void CategorizedListViewPrivate::leftToRightVisualRect(
    const QModelIndex &index, Item &item, const Block &block, const QPoint &blockPos) const
{
    const int firstIndexRow = block.firstIndex.row();
    ...
    if (hasGrid())
    {
        ...
        {
            item.topLeft.rx() =
                (relativeRow % maxItemsPerRow) * q->gridSize().width() + blockPos.x() + categoryDrawer->leftMargin();
        }
        else
        {
            item.topLeft.rx() = viewportWidth() - ((relativeRow % maxItemsPerRow) + 1) * q->gridSize().width() +
                                categoryDrawer->leftMargin() + categorySpacing;
        }
    ...
    else
    {
        if (q->uniformItemSizes())
        {
            ...
            {
                item.topLeft.rx() =
                    (relativeRow % maxItemsPerRow) * itemSize.width() + blockPos.x() + categoryDrawer->leftMargin();
            }
            else
            {
        ...
        else
        {
            ...
            if (index != block.firstIndex)
            {
                ...
                if ((prevRect.bottomRight().x() + 1) + currSize.width() - blockPos.x() + q->spacing() > viewportW)
                {
                    ...
                    {
                        item.topLeft.rx() =
                            viewportWidth() - currSize.width() + categoryDrawer->leftMargin() + categorySpacing;
                    }
...
void CategorizedListViewPrivate::topToBottomVisualRect(
    const QModelIndex &index, Item &item, const Block &block, const QPoint &blockPos) const
{
    const int firstIndexRow = block.firstIndex.row();
...
void CategorizedListViewPrivate::_k_slotCollapseOrExpandClicked(QModelIndex)
{
}

// END: CategorizedListViewPrivate part

// BEGIN: Public part

CategorizedListView::CategorizedListView(QWidget *parent) : QListView(parent), d(new CategorizedListViewPrivate(this))
{
}
...
void CategorizedListView::setModel(QAbstractItemModel *model)
{
    ...
    d->proxyModel = dynamic_cast<CategorizedSortFilterProxyModel *>(model);
...
void CategorizedListView::setGridSize(const QSize &size)
{
    setGridSizeOwn(size);
}

void CategorizedListView::setGridSizeOwn(const QSize &size)
{
    d->regenerateAllElements();
    QListView::setGridSize(size);
}

QRect CategorizedListView::visualRect(const QModelIndex &index) const
{
    if (!d->isCategorized())
    {
    ...
    CategorizedListViewPrivate::Block &block = d->blocks[category];
    ...
    CategorizedListViewPrivate::Item &ritem = block.items[index.row() - firstIndexRow];

    if (ritem.topLeft.isNull() || (block.quarantineStart.isValid() && index.row() >= block.quarantineStart.row()))
    {
        if (flow() == LeftToRight)
        {
    ...
    CategorizedListViewPrivate::Item item(ritem);
    ...
    if (d->hasGrid())
    {
        ...
        QRect res(
            item.topLeft.x() + ((sizeGrid.width() - resultingSize.width()) / 2), item.topLeft.y(),
...
CategorizedListViewPainter *CategorizedListView::categoryDrawer() const
{
    return d->categoryDrawer;
}

void CategorizedListView::setCategoryDrawer(CategorizedListViewPainter *categoryDrawer)
{
    if (d->categoryDrawer)
    {
        disconnect(
            d->categoryDrawer, SIGNAL(collapseOrExpandClicked(QModelIndex)), this,
            SLOT(_k_slotCollapseOrExpandClicked(QModelIndex)));
    }
...
int CategorizedListView::categorySpacing() const
{
    return d->categorySpacing;
}

void CategorizedListView::setCategorySpacing(int categorySpacing)
{
    ...
    for (QHash<QString, CategorizedListViewPrivate::Block>::Iterator it = d->blocks.begin(); it != d->blocks.end();
         ++it)
    {
        CategorizedListViewPrivate::Block &block = *it;
        block.outOfQuarantine = false;
...
bool CategorizedListView::alternatingBlockColors() const
{
    return d->alternatingBlockColors;
}

void CategorizedListView::setAlternatingBlockColors(bool enable)
{
    d->alternatingBlockColors = enable;
}

bool CategorizedListView::collapsibleBlocks() const
{
    return d->collapsibleBlocks;
}

void CategorizedListView::setCollapsibleBlocks(bool enable)
{
    d->collapsibleBlocks = enable;
}

QModelIndexList CategorizedListView::block(const QString &category)
{
    QModelIndexList res;
    const CategorizedListViewPrivate::Block &block = d->blocks[category];
...
QModelIndexList CategorizedListView::block(const QModelIndex &representative)
{
    return block(representative.data(CategorizedSortFilterProxyModel::CategoryDisplayRole).toString());
}

QModelIndex CategorizedListView::indexAt(const QPoint &point) const
{
    if (!d->isCategorized())
    {
...
void CategorizedListView::reset()
{
    d->blocks.clear();
    QListView::reset();
}

void CategorizedListView::paintEvent(QPaintEvent *event)
{
    if (!d->isCategorized())
    {
    ...
    QHash<QString, CategorizedListViewPrivate::Block>::ConstIterator it(d->blocks.constBegin());
    ...
    if (intersecting.first.isValid() && intersecting.second.isValid())
    {
        ...
        QString category;
        CategorizedListViewPrivate::Block *block = 0;
        ...
        while (i <= intersecting.second.row())
        {
            ...
            if (i == indexToCheckIfBlockCollapsed)
            {
                ...
                category = categoryIndex.data(CategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
...
void CategorizedListView::resizeEvent(QResizeEvent *event)
{
    d->regenerateAllElements();
    QListView::resizeEvent(event);
}

void CategorizedListView::setSelection(const QRect &rect, QItemSelectionModel::SelectionFlags flags)
{
    if (!d->isCategorized())
    {
...
void CategorizedListView::mousePressEvent(QMouseEvent *event)
{
...
void CategorizedListView::mouseReleaseEvent(QMouseEvent *event)
{
...
void CategorizedListView::leaveEvent(QEvent *event)
{
...
void CategorizedListView::startDrag(Qt::DropActions supportedActions)
{
...
void CategorizedListView::dragLeaveEvent(QDragLeaveEvent *event)
{
    QListView::dragLeaveEvent(event);
}

void CategorizedListView::dropEvent(QDropEvent *event)
{
...
QModelIndex CategorizedListView::moveCursor(CursorAction cursorAction, Qt::KeyboardModifiers modifiers)
{
    if (!d->isCategorized())
    {
    ...
    case MoveDown:
    {
        if (d->hasGrid() || uniformItemSizes())
        {
            ...
            const CategorizedListViewPrivate::Block &block = d->blocks[d->categoryForIndex(current)];
            ...
            const QModelIndex nextIndex =
                d->proxyModel->index(block.firstIndex.row() + block.items.count(), modelColumn(), rootIndex());
            ...
            const CategorizedListViewPrivate::Block &nextBlock = d->blocks[d->categoryForIndex(nextIndex)];
            ...
            {
                return d->proxyModel->index(
                    nextBlock.firstIndex.row() + currentRelativePos, modelColumn(), rootIndex());
            }
    ...
    case MoveUp:
    {
        if (d->hasGrid() || uniformItemSizes())
        {
            ...
            const CategorizedListViewPrivate::Block &prevBlock = d->blocks[d->categoryForIndex(prevIndex)];
            ...
            {
                return d->proxyModel->index(
                    prevBlock.firstIndex.row() + prevBlock.items.count() - remainder + currentRelativePos,
...
void CategorizedListView::rowsAboutToBeRemoved(const QModelIndex &parent, int start, int end)
{
    if (!d->isCategorized())
    {
...
void CategorizedListView::updateGeometries()
{
    ...
    /*
     * QListView::updateGeometries() has it's own opinion on whether the scrollbars should be visible (valid range) or
     * not
     ...
     *
     * As a result QListView and CategorizedListView occasionally started a race on the scrollbar visibility,
     * effectively blocking the UI
     ...
     *
     * Since the implicit show/hide by the followin range setting will cause further resizes if the policy is
     * Qt::ScrollBarAsNeeded
     * we keep it static until we're done, then restore the original value and ultimately change the scrollbar
     * visibility ourself.
     */

    if (d->isCategorized())   // important! - otherwise we'd pollute the setting if the view is initially not
                              // categorized
    {
        setVerticalScrollBarPolicy(
            (verticalP == Qt::ScrollBarAlwaysOn || verticalScrollBar()->isVisibleTo(this)) ? Qt::ScrollBarAlwaysOn
                                                                                           : Qt::ScrollBarAlwaysOff);
        setHorizontalScrollBarPolicy(
            (horizontalP == Qt::ScrollBarAlwaysOn || horizontalScrollBar()->isVisibleTo(this))
    ...
    //      In theory, as CategorizedListView has been designed, there is no need of horizontal scroll bar.
    horizontalScrollBar()->setRange(0, 0);
...
void CategorizedListView::currentChanged(const QModelIndex &current, const QModelIndex &previous)
{
    QListView::currentChanged(current, previous);
}

void CategorizedListView::dataChanged(
    const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> & = QVector<int>())
{
    ...
    *d->hoveredBlock = CategorizedListViewPrivate::Block();
    ...
    QModelIndex categoryIndex;
    QString category;
    CategorizedListViewPrivate::Block *block;
...
void CategorizedListView::rowsInserted(const QModelIndex &parent, int start, int end)
{
...
void CategorizedListView::slotLayoutChanged()
{
    if (!d->isCategorized())
    {
        return;
