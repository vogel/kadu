...
public:
    ...
    virtual std::map<QString, PluginMetadata> provide() noexcept override
    {
...
private:
    std::unique_ptr<PluginDependencyHandler>
    createPluginDependencyHandler(const QVector<QPair<QString, QStringList>> &plugins);
    SimpleMetadataProvider *
    createMetatadataProvider(const QVector<QPair<QString, QStringList>> &plugins, QObject *parent);
...
std::unique_ptr<PluginDependencyHandler>
PluginDependencyHandlerTest::createPluginDependencyHandler(const QVector<QPair<QString, QStringList>> &plugins)
{
...
SimpleMetadataProvider *PluginDependencyHandlerTest::createMetatadataProvider(
    const QVector<QPair<QString, QStringList>> &plugins, QObject *parent)
{
...
void PluginDependencyHandlerTest::verifyDependencies(
    const PluginDependencyGraph &graph, const QString &pluginName, const QStringList &dependencies,
    const QStringList &dependents)
{
...
void PluginDependencyHandlerTest::selfDependencyTest()
{
    auto handler = createPluginDependencyHandler(
        QVector<QPair<QString, QStringList>>{qMakePair(QString{"p1"}, QStringList{"p1"})});
