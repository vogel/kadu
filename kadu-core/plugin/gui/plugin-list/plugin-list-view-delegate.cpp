...
PluginListWidgetDelegate::PluginListWidgetDelegate(QAbstractItemView *v, QObject *parent)
        : QAbstractItemDelegate(parent), ItemView(0), widgetPool(new PluginListWidgetDelegateWidgets(this)), model(0),
          viewDestroyed(false)
{
    if (!v)
        return;
    ...
    {
        connect(ItemView, SIGNAL(collapsed(QModelIndex)), this, SLOT(initializeModel()));
        connect(ItemView, SIGNAL(expanded(QModelIndex)), this, SLOT(initializeModel()));
    }
}
...
bool PluginListWidgetDelegate::eventFilter(QObject *watched, QEvent *event)
{
    ...
    if (model != ItemView->model())
    {
        ...
        connect(
            model, SIGNAL(rowsInserted(QModelIndex, int, int)), this, SLOT(_k_slotRowsInserted(QModelIndex, int, int)));
        connect(
            model, SIGNAL(rowsAboutToBeRemoved(QModelIndex, int, int)), this,
            SLOT(_k_slotRowsAboutToBeRemoved(QModelIndex, int, int)));
        connect(
            model, SIGNAL(rowsRemoved(QModelIndex, int, int)), this, SLOT(_k_slotRowsRemoved(QModelIndex, int, int)));
        connect(
            model, SIGNAL(dataChanged(QModelIndex, QModelIndex)), this,
