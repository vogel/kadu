...
PluginDependencyGraphBuilder::PluginDependencyGraphBuilder(QObject *parent) : QObject{parent}
{
}
...
QSet<QString> PluginDependencyGraphBuilder::setWithDependents(
    QSet<QString> set, const PluginDependencyGraph &graph, const QString &pluginName) const
{
...
std::set<QString> PluginDependencyGraphBuilder::invalidPlugins(
    const PluginDependencyGraph &graph, const std::map<QString, PluginMetadata> &plugins) const
{
    auto pluginInGraph = graph.plugins();
    auto pluginsWithMetadata = std::set<QString>{};
    std::transform(
        std::begin(plugins), std::end(plugins), std::inserter(pluginsWithMetadata, pluginsWithMetadata.begin()),
        [](const std::map<QString, PluginMetadata>::value_type &v) { return v.first; });

    auto result = std::set<QString>{};
    std::set_difference(
        std::begin(pluginInGraph), std::end(pluginInGraph), std::begin(pluginsWithMetadata),
        std::end(pluginsWithMetadata), std::inserter(result, result.begin()));
    return result;
}
