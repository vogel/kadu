...
void PluginActivationService::activatePlugin(const QString &pluginName)
{
    if (!contains(m_activePlugins, pluginName))
        m_activePlugins.insert(std::make_pair(
            pluginName,
            std::make_unique<ActivePlugin>(
                m_pathsProvider->pluginsLibPath(), m_pathsProvider->dataPath() + QStringLiteral("plugins/translations"),
