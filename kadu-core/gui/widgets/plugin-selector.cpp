...
PluginSelector::Private::Private(PluginSelector *parent)
    : QObject(parent)
    , parent(parent)
    , listView(0)
...
PluginSelector::Private::~Private()
{
    delete categoryDrawer;
}

void PluginSelector::Private::updateDependencies(PluginEntry *pluginEntry, bool added)
{
    if (added) {
...
int PluginSelector::Private::dependantLayoutValue(int value, int width, int totalWidth) const
{
...
PluginSelector::Private::DependenciesWidget::DependenciesWidget(QWidget *parent)
    : QWidget(parent)
...
PluginSelector::Private::DependenciesWidget::~DependenciesWidget()
{
}

void PluginSelector::Private::DependenciesWidget::addDependency(const QString &dependency, const QString &pluginCausant, bool added)
{
    if (!isVisible())
        setVisible(true);
...
void PluginSelector::Private::DependenciesWidget::userOverrideDependency(const QString &dependency)
{
...
void PluginSelector::Private::DependenciesWidget::clearDependencies()
{
    addedByDependencies = 0;
    removedByDependencies = 0;
    dependencyMap.clear();
...
void PluginSelector::Private::DependenciesWidget::showDependencyDetails()
{
...
void PluginSelector::Private::DependenciesWidget::updateDetails()
{
...
void PluginSelector::addPlugins(const QString &componentName,
...
void PluginSelector::addPlugins(const KComponentData &instance,
                                 const QString &categoryName,
                                 const QString &categoryKey,
...
void PluginSelector::addPlugins(const QList<KPluginInfo> &pluginInfoList,
...
bool PluginSelector::isDefault() const
{
    for (int i = 0; i < d->pluginModel->rowCount(); i++) {
        const QModelIndex index = d->pluginModel->index(i, 0);
        PluginEntry *pluginEntry = static_cast<PluginEntry*>(index.internalPointer());
...
void PluginSelector::updatePluginsState()
{
    for (int i = 0; i < d->pluginModel->rowCount(); i++) {
        const QModelIndex index = d->pluginModel->index(i, 0);
        PluginEntry *pluginEntry = static_cast<PluginEntry*>(index.internalPointer());
...
PluginSelector::Private::PluginModel::PluginModel(PluginSelector::Private *pluginSelector_d, QObject *parent)
    : QAbstractListModel(parent)
    , pluginSelector_d(pluginSelector_d)
{
}

PluginSelector::Private::PluginModel::~PluginModel()
{
}

void PluginSelector::Private::PluginModel::addPlugins(const QList<KPluginInfo> &pluginList, const QString &categoryName, const QString &categoryKey, const KConfigGroup &cfgGroup, PluginLoadMethod pluginLoadMethod, bool manuallyAdded)
{
...
QList<KService::Ptr> PluginSelector::Private::PluginModel::pluginServices(const QModelIndex &index) const
{
    return static_cast<PluginEntry*>(index.internalPointer())->pluginInfo.kcmServices();
}

QModelIndex PluginSelector::Private::PluginModel::index(int row, int column, const QModelIndex &parent) const
{
    Q_UNUSED(parent)
...
QVariant PluginSelector::Private::PluginModel::data(const QModelIndex &index, int role) const
{
...
bool PluginSelector::Private::PluginModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    if (!index.isValid()) {
        return false;
    }
...
int PluginSelector::Private::PluginModel::rowCount(const QModelIndex &parent) const
{
    if (parent.isValid()) {
        return 0;
    }
...
PluginSelector::Private::ProxyModel::ProxyModel(PluginSelector::Private *pluginSelector_d, QObject *parent)
...
PluginSelector::Private::ProxyModel::~ProxyModel()
{
}

bool PluginSelector::Private::ProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
...
bool PluginSelector::Private::ProxyModel::subSortLessThan(const QModelIndex &left, const QModelIndex &right) const
{
...
PluginSelector::Private::PluginDelegate::PluginDelegate(PluginSelector::Private *pluginSelector_d, QObject *parent)
...
PluginSelector::Private::PluginDelegate::~PluginDelegate()
{
    delete checkBox;
    delete pushButton;
}

void PluginSelector::Private::PluginDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    if (!index.isValid()) {
        return;
    }
...
QSize PluginSelector::Private::PluginDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    int i = 5;
    int j = 1;
    if (index.model()->data(index, ServicesCountRole).toBool()) {
...
QList<QWidget*> PluginSelector::Private::PluginDelegate::createItemWidgets() const
{
...
void PluginSelector::Private::PluginDelegate::updateItemWidgets(const QList<QWidget*> widgets,
...
void PluginSelector::Private::PluginDelegate::slotStateChanged(bool state)
{
...
void PluginSelector::Private::PluginDelegate::emitChanged()
{
    emit changed(true);
}

void PluginSelector::Private::PluginDelegate::slotAboutClicked()
{
    const QModelIndex index = focusedIndex();
...
void PluginSelector::Private::PluginDelegate::slotConfigureClicked()
{
    const QModelIndex index = focusedIndex();
    const QAbstractItemModel *model = index.model();
...
void PluginSelector::Private::PluginDelegate::slotDefaultClicked()
{
    foreach (KCModuleProxy *moduleProxy, moduleProxyList) {
...
QFont PluginSelector::Private::PluginDelegate::titleFont(const QFont &baseFont) const
{
