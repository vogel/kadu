...
void AccountBuddyListWidget::startImportTransfer()
{
	...
	{
		MessageDialog::msg(tr("Cannot import user list from server in offline mode"), false, "dialog-error.png", this);
		return;
	}
...
void AccountBuddyListWidget::startExportTransfer()
{
	...
	if (!CurrentAccount.protocolHandler()->isConnected())
	{
		MessageDialog::msg(tr("Cannot export user list to server in offline mode"), false, "dialog-error.png", this);
		kdebugf2();
		return;
	}
...
void AccountBuddyListWidget::buddiesListImported(bool ok, BuddyList buddies)
{
	...
	if (!ok)
	{
	  	MessageDialog *m = new MessageDialog(tr("Contacts list couldn't be downloaded. Please check that account %0 is connected.").arg(CurrentAccount.id()), MessageDialog::RETRY|MessageDialog::CANCEL, true, "dialog-error.png", this);
...
void AccountBuddyListWidget::buddiesListExported(bool ok)
{
	...
	{
		if (!ok)
			MessageDialog::msg(tr("The application encountered an internal error\nThe delete userlist on server was unsuccessful"), false, "dialog-error.png", this);
	}
	else
	{
		if (!ok)
			MessageDialog::msg(tr("The application encountered an internal error\nThe export was unsuccessful"), false, "dialog-error.png", this);
	}
