...
QStyleOptionViewItemV4 CategorizedListViewPrivate::blockRect(const QModelIndex &representative)
{
        ...
        const QString categoryDisplay = representative.data(CategorizedSortFilterProxyModel::CategoryDisplayRole).toString();
...
void CategorizedListViewPrivate::leftToRightVisualRect(const QModelIndex &index, Item &item,
...
void CategorizedListViewPrivate::topToBottomVisualRect(const QModelIndex &index, Item &item,
                const Block &block, const QPoint &blockPos) const
{
        const int firstIndexRow = block.firstIndex.row();
...
CategorizedListView::CategorizedListView(QWidget *parent)
                : QListView(parent)
                , d(new CategorizedListViewPrivate(this))
{
}
...
void CategorizedListView::setCategorySpacing(int categorySpacing)
{
        ...
        for (QHash<QString, CategorizedListViewPrivate::Block>::Iterator it = d->blocks.begin(); it != d->blocks.end(); ++it)
        {
                CategorizedListViewPrivate::Block &block = *it;
                block.outOfQuarantine = false;
        }
...
void CategorizedListView::setSelection(const QRect &rect,
...
QModelIndex CategorizedListView::moveCursor(CursorAction cursorAction,
...
void CategorizedListView::updateGeometries()
{
         ...
         *
         * As a result QListView and CategorizedListView occasionally started a race on the scrollbar visibility, effectively blocking the UI
...
void CategorizedListView::currentChanged(const QModelIndex &current,
...
void CategorizedListView::rowsInserted(const QModelIndex &parent,
                                       int start,
                                       int end)
{
