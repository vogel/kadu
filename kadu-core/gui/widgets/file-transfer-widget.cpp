...
void FileTransferWidget::createGui()
{
	...
	{
		icon->setPixmap(IconsManager::instance()->iconByPath("kadu_icons/kadu-transfer_send").pixmap(16, 16));
	...
	{
		icon->setPixmap(IconsManager::instance()->iconByPath("kadu_icons/kadu-transfer_receive").pixmap(16, 16));
...
void FileTransferWidget::removeTransfer()
{
	...
	{
		if (!MessageDialog::ask("", tr("Kadu"), tr("Are you sure you want to remove this transfer?"), this))
			return;
		else
			if (handler())
				handler()->stop();
