...
FileTransferWindow::FileTransferWindow(QWidget *parent) : QFrame{parent}, DesktopAwareObject{this}
{
}
...
void FileTransferWindow::init()
{
    ...
    createGui();
    new WindowGeometryManager{new ConfigFileVariantWrapper{m_configuration, QStringLiteral("General"),
                                                           QStringLiteral("TransferWindowGeometry")},
                              QRect(200, 200, 500, 300), this};

    for (auto &&fileTransfer : m_fileTransferManager->items())
        fileTransferAdded(fileTransfer);

    connect(
        m_fileTransferManager, SIGNAL(fileTransferAdded(FileTransfer)), this, SLOT(fileTransferAdded(FileTransfer)));
    connect(
        m_fileTransferManager, SIGNAL(fileTransferRemoved(FileTransfer)), this,
