...
FileTransferWidget::FileTransferWidget(FileTransfer transfer, QWidget *parent)
        : QWidget{parent}, m_transfer{std::move(transfer)}, m_speed{0}
{
}
...
bool FileTransferWidget::canStop() const
{
    ...
    if (FileTransferStatus::WaitingForConnection == m_transfer.transferStatus())
        return true;
    if (FileTransferDirection::Outgoing == m_transfer.transferDirection() &&
        FileTransferStatus::WaitingForAccept == m_transfer.transferStatus())
        return true;
    if (FileTransferStatus::Transfer == m_transfer.transferStatus())
        return true;
    return false;
...
void FileTransferWidget::openFolder()
{
    if (canOpenFolder())
        QDesktopServices::openUrl(
            QUrl::fromLocalFile(QFileInfo(m_transfer.localFileName()).absoluteDir().absolutePath()));
}
...
void FileTransferWidget::remove()
{
    ...
    {
        auto dialog =
            MessageDialog::create(QIcon{}, tr("Kadu"), tr("Are you sure you want to remove this transfer?"), this);
...
{
    if (!m_transfer.error().isEmpty() || FileTransferStatus::Transfer != m_transfer.transferStatus() ||
        !m_lastUpdateTime.isValid())
    {
        m_speed = 0;
