...
static void appendStr(QString &str, const QString &res, int length)
{
    length -= res.length();
    while (length-- > 0)
        str += QChar(' ');
...
static void appendInt(QString &str, int number, int length)
{
    int n = number;
    length--;
    while (n /= 10)
...
static void finishStr(
    SystemInfo *systemInfo, QString &str, const WeekDate &week_date, const QDate &date, const QTime &time, QChar c,
    int length)
{
    if (length <= 0)
        return;
