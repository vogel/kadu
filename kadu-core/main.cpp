...
int main(int argc, char *argv[]) try
{
    ...
    auto profileDirectory = executionArguments.profileDirectory().isEmpty() ? QString::fromUtf8(qgetenv("CONFIG_DIR"))
    ...
    try
    {
        injector.instantiate<Application>();   // force creation of Application
                                               // object
    ...
    catch (ConfigurationUnusableException &)
    {
        ...
        auto errorMessage = QCoreApplication::translate(
                                "@default",
                                "We're sorry, but Kadu cannot be loaded. "
                                "Profile is inaccessible. Please check permissions in the '%1' "
                                "directory.")
                                .arg(profilePath.left(profilePath.length() - 1));
        QMessageBox::critical(
            0, QCoreApplication::translate("@default", "Profile Inaccessible"), errorMessage, QMessageBox::Abort);
