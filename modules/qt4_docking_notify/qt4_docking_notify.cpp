...
Qt4Notify::Qt4Notify() :
		Notifier("Tray Icon Balloon", "Tray Icon Balloon", IconsManager::instance()->iconByPath("external_modules/module_qt4notify"))
{
	kdebugf();
...
QString Qt4Notify::parseText(const QString &text, Notification *notification, const QString &def)
{
	...
	{
		if (chatNotification)
			ret = Parser::parse(text, *chatNotification->chat().contacts().begin(), notification);
		else
