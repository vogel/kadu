...
void HintsConfigurationWidget::saveNotifyConfigurations()
{
	...
	foreach(const HintProperties &hintProperty, hintProperties)
	{
		...
		config_file.writeEntry("OSDHints", QString("Event_") + eventName + "_font", hintProperty.font);
		config_file.writeEntry("OSDHints", QString("Event_") + eventName + "_fgcolor", hintProperty.foregroundColor);
		config_file.writeEntry("OSDHints", QString("Event_") + eventName + "_bgcolor", hintProperty.backgroundColor);
		config_file.writeEntry("OSDHints", QString("Event_") + eventName + "_timeout", (int)hintProperty.timeout);
		config_file.writeEntry("OSDHints", QString("Event_") + eventName + "_syntax", hintProperty.syntax);
	}
}
...
void HintsConfigurationWidget::switchToEvent(const QString &event)
{
	...
	else
	{
		...
		config_file.addVariable("OSDHints", QString("Event_") + event + "_font", *defaultFont);
		config_file.addVariable("OSDHints", QString("Event_") + event + "_fgcolor", paletteForegroundColor());
		config_file.addVariable("OSDHints", QString("Event_") + event + "_bgcolor", paletteBackgroundColor());

		currentProperties.font = config_file.readFontEntry("OSDHints", QString("Event_") + event + "_font", defaultFont);
		currentProperties.foregroundColor = config_file.readColorEntry("OSDHints", QString("Event_") + event + "_fgcolor");
		currentProperties.backgroundColor = config_file.readColorEntry("OSDHints", QString("Event_") + event + "_bgcolor");
		currentProperties.timeout = config_file.readUnsignedNumEntry("OSDHints", QString("Event_") + event + "_timeout", 10);
		currentProperties.syntax = config_file.readEntry("OSDHints", QString("Event_") + event + "_syntax");
	}
