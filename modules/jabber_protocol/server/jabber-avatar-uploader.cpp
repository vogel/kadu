...
void JabberAvatarUploader::fetchingVCardFinished()
{
	XMPP::Jid jid = XMPP::Jid(MyAccount.id());
	QByteArray ba;
        QBuffer buffer(&ba);
...
void JabberAvatarUploader::uploadingVCardFinished()
{
...
QByteArray JabberAvatarUploader::scaleAvatar(const QByteArray& b)
{
	int maxSize = 96;
...
void JabberAvatarUploader::itemPublished(const XMPP::Jid& jid, const QString& n, const XMPP::PubSubItem& item)
{
	Q_UNUSED(jid)
...
void JabberAvatarUploader::publish_success(const QString& n, const XMPP::PubSubItem& item)
{
...
void JabberAvatarUploader::publish_error(const QString& n, const XMPP::PubSubItem& item)
{
	if (n == "http://www.xmpp.org/extensions/xep-0084.html#ns-data" && item.id() == selfAvatarHash_)
	{
		/*
