...
EncryptionManager* old_encryption_manager;

extern "C" KADU_EXPORT int encryption_old_init(bool firstLoad)
{
	old_encryption_manager = new EncryptionManager(firstLoad);
	MainConfigurationWindow::registerUiFile(dataPath("kadu/modules/configuration/encryption_old.ui"), old_encryption_manager);

	return 0;
}

extern "C" KADU_EXPORT void encryption_old_close()
{
	MainConfigurationWindow::unregisterUiFile(dataPath("kadu/modules/configuration/encryption_old.ui"), old_encryption_manager);

	delete old_encryption_manager;
	old_encryption_manager = 0;
}
