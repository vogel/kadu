...
#ifndef Q_OS_WIN // TODO: do we realy need this headers od *nix?
#include <sys/types.h>
...
void disableNonVoiceUles(KaduAction *action)
{
	...
	if (user.usesProtocol("Gadu") && user.ID("Gadu").toUInt() != myUin && user.status("Gadu").isAvailable())
	{
...
bool VoiceChatDialog::socketEvent(DccSocket *socket, bool &lock)
{
	...
	switch (socket->ggDccEvent()->type)
	{
		case GG_EVENT_DCC_VOICE_DATA:
		{
			...
			//zostawiamy zapas gdyby to si� kiedy� zmieni�o
...
{
	wsem = new QSemaphore(32);	//mo�e by� max 32 pr�bek w kolejce
}
...
void PlayThread::addGsmSample(char *data, int length)
{
	...
	if (samples.size() >= 3)
	{
		...
		// jak nie b�dziemy takich rzeczy robi�, to lag b�dzie si� powi�ksza� :/
		while (!samples.empty())
		{
...
/*	if (silence == 0)
	...
	/* celowo sprawdzane jest to PO kodowaniu, bo koder musi
	   zna� ca�o�� sygna�u �eby poprawnie pracowa� */
	if (silence == 160 * 10)
	{
